      ...
      <div v-for="cat in categories" :key="cat.id" class="p-4 flex items-center justify-between group hover:bg-white/5 transition-colors">
        <div v-if="editingId === cat.id" class="flex-grow flex gap-2">
            <input
              ...
              v-focus
            >
            <button class="p-2 text-emerald-600 hover:bg-emerald-500/10 rounded-xl transition-all" @click="saveEdit">
                <Check class="h-4 w-4" />
            </button>
            <button class="p-2 text-red-600 hover:bg-red-500/10 rounded-xl transition-all" @click="cancelEdit">
                <X class="h-4 w-4" />
            </button>
        </div>
        <template v-else>
