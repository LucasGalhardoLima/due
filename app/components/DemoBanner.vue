...
<template>
  <div 
    v-if="isDemoMode"
    class="bg-warning text-warning-foreground py-2 px-4 flex items-center justify-between sticky top-0 z-[100] shadow-lg animate-in fade-in slide-in-from-top duration-300 border-b border-warning/30"
  >
    <div class="flex items-center gap-3">
        <div class="relative">
          <ShieldCheck class="w-5 h-5" />
          <span class="absolute -top-1 -right-1 w-2 h-2 bg-warning-foreground rounded-full animate-ping"/>
        </div>
    ...
    <button
        class="flex items-center gap-2 bg-warning-foreground/10 hover:bg-warning-foreground/20 px-3 py-1 rounded-full text-xs font-bold transition-all border border-warning-foreground/20"
        @click="exitDemoMode"
    >
        Sair do Modo Demo
        <X class="w-3 h-3" />
